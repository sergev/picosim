...
void Registers::dump()
{
    ...
    dump_reg("  r8: ", register_bank[8]); std::cout << std::endl;

    dump_reg("  r1: ", register_bank[1]);
    dump_reg("     r9: ", register_bank[9]); std::cout << std::endl;

    dump_reg("  r2: ", register_bank[2]);
    dump_reg("    r10 (sl): ", register_bank[10]); std::cout << std::endl;

    dump_reg("  r3: ", register_bank[3]);
    dump_reg("    r11 (fp): ", register_bank[11]); std::cout << std::endl;

    dump_reg("  r4: ", register_bank[4]);
    dump_reg("    r12 (ip): ", register_bank[12]); std::cout << std::endl;

    dump_reg("  r5: ", register_bank[5]);
    dump_reg("    r13 (sp): ", register_bank[13]); std::cout << std::endl;

    dump_reg("  r6: ", register_bank[6]);
    dump_reg("    r14 (lr): ", register_bank[14]); std::cout << std::endl;

    dump_reg("  r7: ", register_bank[7]);
    dump_reg("    r15 (pc): ", register_bank[15]); std::cout << std::endl;
...
void Registers::setValue(int reg_num, int32_t value)
{
    if (reg_num < 16) {
        ...
        if (Log::is_verbose()) {
            Log::out() << "          " << reg_name[reg_num] << " = " << std::hex
