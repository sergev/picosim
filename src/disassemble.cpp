...
int arm_evaluate_opcode(unsigned opcode, unsigned address, struct arm_instruction *instruction);
int thumb_evaluate_opcode(unsigned short opcode, unsigned address,
                          struct arm_instruction *instruction);
int thumb2_opcode(unsigned address, struct arm_instruction *instruction,
                  unsigned short (*read_u16)(unsigned));
...
static int evaluate_unknown(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_pld(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    /* PLD */
...
static int evaluate_srs(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    ...
    case 0x08400000:
        snprintf(instruction->text, 128,
                 ...
                 "\t0x%8.8x"
                 "\tSRS%s\tSP%s, #%d",
                 address, opcode, mode, wback, (unsigned)(opcode & 0x1f));
        break;
    case 0x08100000:
        snprintf(instruction->text, 128,
                 "0x%8.8x"
                 "\t0x%8.8x"
                 "\tRFE%s\tr%d%s",
                 address, opcode, mode, (unsigned)((opcode >> 16) & 0xf), wback);
        break;
    default:
        return evaluate_unknown(opcode, address, instruction);
    }
...
static int evaluate_swi(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    instruction->type = ARM_SWI;

    snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tSVC %#6.6x", address, opcode,
             (opcode & 0xffffff));

    return 0;
}

static int evaluate_blx_imm(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    int offset;
    unsigned immediate;
    unsigned target_address;
    ...
    snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tBLX 0x%8.8x", address, opcode,
             target_address);
...
static int evaluate_b_bl(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    unsigned char L;
    unsigned immediate;
    int offset;
...
static int evaluate_ldc_stc_mcrr_mrrc(unsigned opcode, unsigned address,
                                      struct arm_instruction *instruction)
{
    unsigned char cp_num = (opcode & 0xf00) >> 8;

    /* MCRR or MRRC */
    if (((opcode & 0x0ff00000) == 0x0c400000) || ((opcode & 0x0ff00000) == 0x0c400000)) {
        unsigned char cp_opcode, Rd, Rn, CRm;
        const char *mnemonic;
        ...
        snprintf(instruction->text, 128,
                 ...
                 ((opcode & 0xf0000000) == 0xf0000000) ? "2" : COND(opcode), COND(opcode), cp_num,
                 cp_opcode, Rd, Rn, CRm);
    } else /* LDC or STC */
    {
        unsigned char CRd, Rn, offset;
        unsigned char U;
        const char *mnemonic;
        ...
        if ((opcode & 0x01200000) == 0x01000000) /* offset */
            snprintf(addressing_mode, 32, "[r%i, #%s%d]", Rn, U ? "" : "-", offset);
            ...
            snprintf(addressing_mode, 32, "[r%i], {%d}", Rn, offset >> 2);
        ...
        snprintf(instruction->text, 128,
                 ...
                 "\t%s%s%s p%i, c%i, %s",
                 address, opcode, mnemonic,
                 ((opcode & 0xf0000000) == 0xf0000000) ? "2" : COND(opcode),
...
/* both normal and extended instruction space (condition field b1111) */
static int evaluate_cdp_mcr_mrc(unsigned opcode, unsigned address,
                                struct arm_instruction *instruction)
{
    const char *cond;
    const char *mnemonic;
    unsigned char cp_num, opcode_1, CRd_Rd, CRn, CRm, opcode_2;
    ...
    if (opcode & 0x00000010) /* bit 4 set -> MRC/MCR */
    {
        ...
        snprintf(instruction->text, 128,
                 "0x%8.8x\t0x%8.8x\t%s%s p%i, 0x%2.2x, r%i, c%i, c%i, 0x%2.2x", address, opcode,
                 mnemonic, cond, cp_num, opcode_1, CRd_Rd, CRn, CRm, opcode_2);
    } else /* bit 4 not set -> CDP */
    {
        ...
        snprintf(instruction->text, 128,
                 "0x%8.8x\t0x%8.8x\t%s%s p%i, 0x%2.2x, c%i, c%i, c%i, 0x%2.2x", address, opcode,
                 mnemonic, cond, cp_num, opcode_1, CRd_Rd, CRn, CRm, opcode_2);
    }
...
/* Load/store instructions */
static int evaluate_load_store(unsigned opcode, unsigned address,
                               struct arm_instruction *instruction)
{
    unsigned char I, P, U, B, W, L;
    unsigned char Rn, Rd;
    const char *operation; /* "LDR" or "STR" */
    const char *suffix;    /* "", "B", "T", "BT" */
    char offset[32];
    ...
    if (!I) /* #+-<offset_12> */
    {
        unsigned offset_12 = (opcode & 0xfff);
        if (offset_12)
    ...
    if (P == 1) {
        if (W == 0) /* offset */
        {
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s%s r%i, [r%i%s]", address,
                     opcode, operation, COND(opcode), suffix, Rd, Rn, offset);
    ...
    {
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s%s r%i, [r%i]%s", address, opcode,
                 operation, COND(opcode), suffix, Rd, Rn, offset);
...
static int evaluate_extend(unsigned opcode, unsigned address, char *cp)
{
    ...
    if (rn == 0xf) {
        sprintf(cp, "%cXT%s%s\tr%d, r%d%s", (opcode & (1 << 22)) ? 'U' : 'S', type, COND(opcode),
                rd, rm, rot);
        return ARM_MOV;
    } else {
        sprintf(cp, "%cXTA%s%s\tr%d, r%d, r%d%s", (opcode & (1 << 22)) ? 'U' : 'S', type,
                COND(opcode), rd, rn, rm, rot);
        return ARM_ADD;
    }
}

static int evaluate_p_add_sub(unsigned opcode, unsigned address, char *cp)
{
    const char *prefix;
    const char *op;
    int type;
    ...
    sprintf(cp, "%s%s%s\tr%d, r%d, r%d", prefix, op, COND(opcode), (int)(opcode >> 12) & 0xf,
...
static int evaluate_media(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    char *cp = instruction->text;
    const char *mnemonic = NULL;

    sprintf(cp, "0x%8.8x\t0x%8.8x\t", address, opcode);
    cp = strchr(cp, 0);
    ...
    if ((opcode & 0x01f00020) == 0x00800000) {
        ...
        sprintf(cp, "PKH%s%s\tr%d, r%d, r%d, %s #%d", type, COND(opcode), (int)(opcode >> 12) & 0xf,
                (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf, shift, imm);
        return 0;
    }
    ...
    if ((opcode & 0x01a00020) == 0x00a00000) {
        ...
        sprintf(cp, "%cSAT%s\tr%d, #%d, r%d, %s #%d", (opcode & (1 << 22)) ? 'U' : 'S',
                COND(opcode), (int)(opcode >> 12) & 0xf, (int)(opcode >> 16) & 0x1f,
                (int)(opcode >> 0) & 0xf, shift, imm);
        return 0;
    }
    ...
    if ((opcode & 0x01f00080) == 0x01000000) {
        ...
        if (rn != 0xf)
            sprintf(cp, "SML%cD%s%s\tr%d, r%d, r%d, r%d", (opcode & (1 << 6)) ? 'S' : 'A',
                    (opcode & (1 << 5)) ? "X" : "", COND(opcode), (int)(opcode >> 16) & 0xf,
                    (int)(opcode >> 0) & 0xf, (int)(opcode >> 8) & 0xf, rn);
        else
            sprintf(cp, "SMU%cD%s%s\tr%d, r%d, r%d", (opcode & (1 << 6)) ? 'S' : 'A',
                    (opcode & (1 << 5)) ? "X" : "", COND(opcode), (int)(opcode >> 16) & 0xf,
                    (int)(opcode >> 0) & 0xf, (int)(opcode >> 8) & 0xf);
        return 0;
    }
    if ((opcode & 0x01f00000) == 0x01400000) {
        sprintf(cp, "SML%cLD%s%s\tr%d, r%d, r%d, r%d", (opcode & (1 << 6)) ? 'S' : 'A',
                (opcode & (1 << 5)) ? "X" : "", COND(opcode), (int)(opcode >> 12) & 0xf,
                (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf, (int)(opcode >> 8) & 0xf);
        return 0;
    }
    if ((opcode & 0x01f00000) == 0x01500000) {
        ...
        if (rn != 0xf)
            sprintf(cp, "SMML%c%s%s\tr%d, r%d, r%d, r%d", (opcode & (1 << 6)) ? 'S' : 'A',
                    (opcode & (1 << 5)) ? "R" : "", COND(opcode), (int)(opcode >> 16) & 0xf,
                    (int)(opcode >> 0) & 0xf, (int)(opcode >> 8) & 0xf, rn);
        else
            sprintf(cp, "SMMUL%s%s\tr%d, r%d, r%d", (opcode & (1 << 5)) ? "R" : "", COND(opcode),
                    (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf, (int)(opcode >> 8) & 0xf);
        return 0;
    }
    ...
    case 0x00e00030:
        ...
        sprintf(cp, "%cSAT16%s\tr%d, #%d, r%d", (opcode & (1 << 22)) ? 'U' : 'S', COND(opcode),
                (int)(opcode >> 12) & 0xf, (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf);
        return 0;
    case 0x00b00030:
        mnemonic = "REV";
        break;
    ...
    case 0x008000b0:
        /* select bytes */
        sprintf(cp, "SEL%s\tr%d, r%d, r%d", COND(opcode), (int)(opcode >> 12) & 0xf,
                (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf);
        return 0;
    case 0x01800010:
        /* unsigned sum of absolute differences */
        if (((opcode >> 12) & 0xf) == 0xf)
            sprintf(cp, "USAD8%s\tr%d, r%d, r%d", COND(opcode), (int)(opcode >> 16) & 0xf,
                    (int)(opcode >> 0) & 0xf, (int)(opcode >> 8) & 0xf);
        else
            sprintf(cp, "USADA8%s\tr%d, r%d, r%d, r%d", COND(opcode), (int)(opcode >> 16) & 0xf,
                    (int)(opcode >> 0) & 0xf, (int)(opcode >> 8) & 0xf, (int)(opcode >> 12) & 0xf);
        return 0;
    }
    if (mnemonic) {
        unsigned rm = (opcode >> 0) & 0xf;
...
static int evaluate_misc_load_store(unsigned opcode, unsigned address,
                                    struct arm_instruction *instruction)
{
    unsigned char P, U, I, W, L, S, H;
    unsigned char Rn, Rd;
    ...
    {
        unsigned offset_8 = ((opcode & 0xf00) >> 4) | (opcode & 0xf);
...
static int evaluate_ldm_stm(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    unsigned char P, U, S, W, L, Rn;
    unsigned register_list;
    ...
    snprintf(instruction->text, 128,
             "0x%8.8x\t0x%8.8x"
             "\t%s%s%s r%i%s, {%s}%s",
             address, opcode, mnemonic, addressing_mode, COND(opcode), Rn, (W) ? "!" : "", reg_list,
             (S) ? "^" : "");
...
static int evaluate_mul_and_extra_ld_st(unsigned opcode, unsigned address,
                                        struct arm_instruction *instruction)
{
    ...
    if ((opcode & 0x000000f0) == 0x00000090) {
        ...
        if ((opcode & 0x0f800000) == 0x00000000) {
            ...
            if (opcode & 0x00200000) {
                ...
                snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tMUL%s%s r%i, r%i, r%i", address,
                         opcode, COND(opcode), (S) ? "S" : "", Rd, Rm, Rs);
            }
        ...
        if ((opcode & 0x0f800000) == 0x00800000) {
            const char *mnemonic = NULL;
            unsigned char Rm, Rs, RdHi, RdLow, S;
            Rm = opcode & 0xf;
            Rs = (opcode & 0xf00) >> 8;
            ...
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s%s r%i, r%i, r%i, r%i", address,
                     opcode, mnemonic, COND(opcode), (S) ? "S" : "", RdLow, RdHi, Rm, Rs);
        ...
        if ((opcode & 0x0f800000) == 0x01000000) {
            ...
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s r%i, r%i, [r%i]", address,
                     opcode, (opcode & 0x00400000) ? "SWPB" : "SWP", COND(opcode), Rd, Rm, Rn);
            return 0;
        }
    }
...
static int evaluate_mrs_msr(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    ...
    if (opcode & 0x00200000) {
        ...
        if (opcode & 0x02000000) {
            unsigned char immediate = (opcode & 0xff);
            unsigned char rotate = (opcode & 0xf00);

            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tMSR%s %s_%s%s%s%s, 0x%8.8x",
                     address, opcode, COND(opcode), PSR, (opcode & 0x10000) ? "c" : "",
                     (opcode & 0x20000) ? "x" : "", (opcode & 0x40000) ? "s" : "",
                     (opcode & 0x80000) ? "f" : "", ror(immediate, (rotate * 2)));
        } else /* register variant */
        {
            unsigned char Rm = opcode & 0xf;
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tMSR%s %s_%s%s%s%s, r%i", address,
                     opcode, COND(opcode), PSR, (opcode & 0x10000) ? "c" : "",
                     (opcode & 0x20000) ? "x" : "", (opcode & 0x40000) ? "s" : "",
                     (opcode & 0x80000) ? "f" : "", Rm);
        }
    ...
    } else /* Move status register to register (MRS) */
    {
        ...
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tMRS%s r%i, %s", address, opcode,
                 COND(opcode), Rd, PSR);
    }
...
/* Miscellaneous instructions */
static int evaluate_misc_instr(unsigned opcode, unsigned address,
                               struct arm_instruction *instruction)
{
    /* MRS/MSR */
    ...
    if ((opcode & 0x006000f0) == 0x00200010) {
        ...
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tBX%s r%i", address, opcode,
                 COND(opcode), Rm);
    ...
    if ((opcode & 0x006000f0) == 0x00200020) {
        ...
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tBXJ%s r%i", address, opcode,
                 COND(opcode), Rm);
    ...
    if ((opcode & 0x006000f0) == 0x00600010) {
        ...
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tCLZ%s r%i, r%i", address, opcode,
                 COND(opcode), Rd, Rm);
    }
    ...
    if ((opcode & 0x006000f0) == 0x00200030) {
        ...
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tBLX%s r%i", address, opcode,
                 COND(opcode), Rm);
    ...
    if ((opcode & 0x0000000f0) == 0x00000050) {
        ...
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s r%i, r%i, r%i", address, opcode,
                 mnemonic, COND(opcode), Rd, Rm, Rn);
    }
    ...
    if ((opcode & 0x0000000f0) == 0x00000070) {
        ...
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tBKPT 0x%4.4x", address, opcode,
                 immediate);
    }
    ...
    if ((opcode & 0x000000090) == 0x00000080) {
        ...
        if ((opcode & 0x00600000) == 0x00000000) {
            ...
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tSMLA%s%s%s r%i, r%i, r%i, r%i",
                     address, opcode, (x) ? "T" : "B", (y) ? "T" : "B", COND(opcode), Rd, Rm, Rs,
                     Rn);
        }

        /* SMLAL < x><y> */
        if ((opcode & 0x00600000) == 0x00400000) {
            unsigned char RdLow, RdHi, Rm, Rs;
            instruction->type = ARM_SMLAxy;
            ...
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tSMLA%s%s%s r%i, r%i, r%i, r%i",
                     address, opcode, (x) ? "T" : "B", (y) ? "T" : "B", COND(opcode), RdLow, RdHi,
                     Rm, Rs);
        }
        ...
        if (((opcode & 0x00600000) == 0x00100000) && (x == 0)) {
            ...
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tSMLAW%s%s r%i, r%i, r%i, r%i",
                     address, opcode, (y) ? "T" : "B", COND(opcode), Rd, Rm, Rs, Rn);
        }

        /* SMUL < x><y> */
        if ((opcode & 0x00600000) == 0x00300000) {
            unsigned char Rd, Rm, Rs;
            instruction->type = ARM_SMULxy;
            Rd = (opcode & 0xf0000) >> 16;
            Rm = (opcode & 0xf);
            Rs = (opcode & 0xf00) >> 8;

            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tSMULW%s%s%s r%i, r%i, r%i", address,
                     opcode, (x) ? "T" : "B", (y) ? "T" : "B", COND(opcode), Rd, Rm, Rs);
        }
        ...
        if (((opcode & 0x00600000) == 0x00100000) && (x == 1)) {
            ...
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tSMULW%s%s r%i, r%i, r%i", address,
                     opcode, (y) ? "T" : "B", COND(opcode), Rd, Rm, Rs);
        }
    }
...
static int evaluate_data_proc(unsigned opcode, unsigned address,
                              struct arm_instruction *instruction)
{
    unsigned char I, op, S, Rn, Rd;
    const char *mnemonic = NULL;
    ...
    {
        unsigned char immed_8 = opcode & 0xff;
        unsigned char rotate_imm = (opcode & 0xf00) >> 8;
        unsigned immediate;
    ...
    {
        unsigned char shift, Rm;
        shift = (opcode & 0x60) >> 5;
        Rm = (opcode & 0xf);

        if ((opcode & 0x10) !=
            0x10) /* Immediate shifts ("<Rm>" or "<Rm>, <shift> #<shift_immediate>") */
        {
            unsigned char shift_imm;
            shift_imm = (opcode & 0xf80) >> 7;
        ...
        {
            unsigned char Rs = (opcode & 0xf00) >> 8;
    ...
    if ((op < 0x8) || (op == 0xc) ||
        (op == 0xe)) /* <opcode3>{<cond>}{S} <Rd>, <Rn>, <shifter_operand> */
    {
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s%s r%i, r%i, %s", address, opcode,
                 mnemonic, COND(opcode), (S) ? "S" : "", Rd, Rn, shifter_operand);
    } else if ((op == 0xd) || (op == 0xf)) /* <opcode1>{<cond>}{S} <Rd>, <shifter_operand> */
    {
        if (opcode == 0xe1a00000) /* print MOV r0,r0 as NOP */
            ...
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s%s r%i, %s", address, opcode,
                     mnemonic, COND(opcode), (S) ? "S" : "", Rd, shifter_operand);
    } else /* <opcode2>{<cond>} <Rn>, <shifter_operand> */
    {
        snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\t%s%s r%i, %s", address, opcode,
                 mnemonic, COND(opcode), Rn, shifter_operand);
    }
...
int arm_evaluate_opcode(unsigned opcode, unsigned address, struct arm_instruction *instruction)
{
    ...
    if ((opcode & 0xf0000000) == 0xf0000000) {
        ...
        if ((opcode & 0x0f000000) == 0x0f000000) {
            instruction->type = ARM_UNDEFINED_INSTRUCTION;
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tUNDEFINED INSTRUCTION", address,
                     opcode);
            return 0;
        }
    }
    ...
    if ((opcode & 0x0e000000) == 0x06000000) {
        ...
        if ((opcode & 0x07f000f0) == 0x07f000f0) {
            instruction->type = ARM_UNDEFINED_INSTRUCTION;
            snprintf(instruction->text, 128, "0x%8.8x\t0x%8.8x\tUNDEF", address, opcode);
            return 0;
        }
    ...
    uerror("ARM: should never reach this point (opcode=%08x)", (unsigned)opcode);
    return -1;
}
                          ...
                          struct arm_instruction *instruction)
{
    ...
    if ((opcode & 0xf000) == 0xb000) {
        ...
        case 0xf:
            if (opcode & 0x000f)
                return evaluate_ifthen_thumb(opcode, address, instruction);
            else
                return evaluate_hint_thumb(opcode, address, instruction);
        }
    ...
    uerror("Thumb: should never reach this point (opcode=%04x)", opcode);
    return -1;
}
                     ...
                     char *cp)
{
    ...
    sprintf(cp, "%s\t%#8.8x", (opcode & (1 << 14)) ? "BL" : "B.W", address);
                       ...
                       char *cp)
{
    ...
    instruction->info.b_bl_bx_blx.reg_operand = -1;
    instruction->info.b_bl_bx_blx.target_address = address;
    sprintf(cp, "B%s.W\t%#8.8x", arm_condition_strings[cond], address);
                       ...
                       char *cp)
{
    ...
    case 0x38:
    case 0x39:
        sprintf(cp, "MSR\t%s, r%d", special_name(opcode & 0xff), (int)(opcode >> 16) & 0x0f);
        return 0;
    case 0x3a:
    ...
    case 0x3e:
    case 0x3f:
        sprintf(cp, "MRS\tr%d, %s", (int)(opcode >> 8) & 0x0f, special_name(opcode & 0xff));
        return 0;
    }
                               ...
                               struct arm_instruction *instruction, char *cp)
{
    ...
    if (one)
        sprintf(cp, "%s%s\tr%d, #%d\t; %#8.8x", mnemonic, suffix2, rd, immed, immed);
    else
        sprintf(cp, "%s%s%s\tr%d, r%d, #%d\t; %#8.8x", mnemonic, suffix, suffix2, rd, rn, immed,
                immed);
                           ...
                           char *cp)
{
    ...
    case 0x1a:
        ...
        sprintf(cp, "%sSAT\tr%d, #%d, r%d, %s #%d\t", is_signed ? "S" : "U", rd,
    ...
    case 0x1c:
        ...
        immed = (opcode >> 6) & 0x03;
        immed |= (opcode >> 10) & 0x1c;
        sprintf(cp, "%sBFX\tr%d, r%d, #%d, #%d\t", is_signed ? "S" : "U", rd, rn, immed,
                (int)(opcode & 0x1f) + 1);
        return 0;
    case 0x16:
        immed = (opcode >> 6) & 0x03;
        ...
        if (rn == 0xf) /* bitfield clear */
            sprintf(cp, "BFC\tr%d, #%d, #%d\t", rd, immed, (int)(opcode & 0x1f) + 1 - immed);
        else /* bitfield insert */
            sprintf(cp, "BFI\tr%d, r%d, #%d, #%d\t", rd, rn, immed,
    ...
    sprintf(cp, "%s\tr%d, r%d, #%d\t; %#3.3x", mnemonic, rd, rn, immed, immed);
    return 0;
...
do_adr:
     ...
     */
    sprintf(cp, "ADR.W\tr%d, %#8.8x", rd, address);
    return 0;
}
                             ...
                             char *cp)
{
    ...
    sprintf(cp, "STR%s.W\tr%d, [r%d, r%d, LSL #%d]", size, rt, rn, (int)opcode & 0x0f,
            (int)(opcode >> 4) & 0x03);
    return 0;

imm12:
    immed = opcode & 0x0fff;
    sprintf(cp, "STR%s.W\tr%d, [r%d, #%u]\t; %#3.3x", size, rt, rn, immed, immed);
    return 0;
...
imm8:
    ...
    sprintf(cp, "STR%s%s\tr%d, [r%d%s, #%s%u%s\t; %#2.2x", size, suffix, rt, rn, p1,
                      ...
                      char *cp)
{
    ...
    case 0:
        if (ra == 0xf)
            sprintf(cp, "MUL\tr%d, r%d, r%d", (int)(opcode >> 8) & 0xf, (int)(opcode >> 16) & 0xf,
                    (int)(opcode >> 0) & 0xf);
        else
            sprintf(cp, "MLA\tr%d, r%d, r%d, r%d", (int)(opcode >> 8) & 0xf,
                    (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf, ra);
        break;
    case 0x10:
        sprintf(cp, "MLS\tr%d, r%d, r%d, r%d", (int)(opcode >> 8) & 0xf, (int)(opcode >> 16) & 0xf,
                (int)(opcode >> 0) & 0xf, ra);
        break;
    default:
        return -1;
                          ...
                          char *cp)
{
    ...
    case 0:
    case 0x20:
        sprintf(cp, "%c%sL\tr%d, r%d, r%d, r%d", (op & 0x20) ? 'U' : 'S', infix,
                (int)(opcode >> 12) & 0xf, (int)(opcode >> 8) & 0xf, (int)(opcode >> 16) & 0xf,
                (int)(opcode >> 0) & 0xf);
        break;
    case 0x1f:
    case 0x3f:
        sprintf(cp, "%cDIV\tr%d, r%d, r%d", (op & 0x20) ? 'U' : 'S', (int)(opcode >> 8) & 0xf,
                (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf);
        break;
    default:
        return -1;
                        ...
                        char *cp)
{
    ...
    case 0:
        ...
        /* FALL THROUGH */
    case 6:
        sprintf(cp, "SRS%s\tsp%s, #%d", mode, t ? "!" : "", (unsigned)(opcode & 0x1f));
        return 0;
    case 1:
        mode = "DB";
        /* FALL THROUGH */
    case 7:
        sprintf(cp, "RFE%s\tr%d%s", mode, (unsigned)((opcode >> 16) & 0xf), t ? "!" : "");
        return 0;
    case 2:
...
strex:
    imm <<= 2;
    if (imm)
        sprintf(cp, "%s\tr%u, r%u, [r%u, #%u]\t; %#2.2x", mnemonic, rd, rt, rn, imm, imm);
    else
        sprintf(cp, "%s\tr%u, r%u, [r%u]", mnemonic, rd, rt, rn);
    return 0;

ldrex:
    imm <<= 2;
    if (imm)
        sprintf(cp, "%s\tr%u, [r%u, #%u]\t; %#2.2x", mnemonic, rt, rn, imm, imm);
    else
        sprintf(cp, "%s\tr%u, [r%u]", mnemonic, rt, rn);
    return 0;
...
immediate:
    ...
    imm <<= 2;
    sprintf(cp, "%s\tr%u, r%u, [r%u%s, #%s%u%s\t; %#2.2x", mnemonic, rt, rd, rn, p1,
...
literal:
    ...
    if (opcode & (1 << 23))
        ...
        address -= imm;
    sprintf(cp, "%s\tr%u, r%u, %#8.8x", mnemonic, rt, rd, address);
    return 0;
}
                           ...
                           char *cp)
{
    ...
    case 2:
        if (rn == 0xf) {
            ...
            case 0:
                if (immed == 0) {
                    sprintf(cp, "MOV%s.W\tr%d, r%d", suffix, rd, (int)(opcode & 0xf));
                    return 0;
                }
                mnemonic = "LSL";
                break;
            ...
            default:
                if (immed == 0) {
                    sprintf(cp, "RRX%s\tr%d, r%d", suffix, rd, (int)(opcode & 0xf));
                    return 0;
                }
                mnemonic = "ROR";
                break;
    ...
    sprintf(cp, "%s%s.W\tr%d, r%d, r%d", mnemonic, suffix, rd, rn, (int)(opcode & 0xf));
...
two:
    sprintf(cp, "%s%s.W\tr%d, r%d", mnemonic, suffix, rn, (int)(opcode & 0xf));
    goto shift;

immediate:
    sprintf(cp, "%s%s.W\tr%d, r%d, #%d", mnemonic, suffix, rd, (int)(opcode & 0xf),
            immed ? immed : 32);
    return 0;
}
                         ...
                         char *cp)
{
    ...
    if (((opcode >> 4) & 0xf) == 0) {
        ...
        if (opcode & (1 << 20))
            suffix = "S";
        sprintf(cp, "%s%s.W\tr%d, r%d, r%d", mnemonic, suffix, (int)(opcode >> 8) & 0xf,
                (int)(opcode >> 16) & 0xf, (int)(opcode >> 0) & 0xf);
        ...
        case 5:
            ...
            sprintf(cp, "%cXT%c.W\tr%d, r%d%s", (opcode & (1 << 24)) ? 'U' : 'S',
        ...
        case 0xb:
            ...
            sprintf(cp, "%s\tr%d, r%d", mnemonic, (int)(opcode >> 8) & 0xf,
                    (int)(opcode >> 0) & 0xf);
            break;
        default:
            return -1;
                          ...
                          char *cp)
{
    ...
    if (rn == 0xf) {
        ...
        if ((opcode & (1 << 23)) == 0)
            immed = -immed;
        sprintf(cp, "LDR\tr%d, %#8.8x", (int)(opcode >> 12) & 0xf, thumb_alignpc4(address) + immed);
        return 0;
    }

    if (opcode & (1 << 23)) {
        immed = opcode & 0x0fff;
        sprintf(cp, "LDR.W\tr%d, [r%d, #%d]\t; %#3.3x", (int)(opcode >> 12) & 0xf, rn, immed,
                immed);
        return 0;
    }

    if (!(opcode & (0x3f << 6))) {
        sprintf(cp, "LDR.W\tr%d, [r%d, r%d, LSL #%d]", (int)(opcode >> 12) & 0xf, rn,
    ...
    if (((opcode >> 8) & 0xf) == 0xe) {
        ...
        sprintf(cp, "LDRT\tr%d, [r%d, #%d]\t; %#2.2x", (int)(opcode >> 12) & 0xf, rn, immed, immed);
        return 0;
    }
    ...
    if (((opcode >> 8) & 0xf) == 0xc || (opcode & 0x0900) == 0x0900) {
        ...
        sprintf(cp, "LDR\tr%d, [r%d%s, #%s%u%s\t; %#2.2x", (int)(opcode >> 12) & 0xf, rn, p1,
                (opcode & 0x200) ? "" : "-", immed, p2, immed);
        return 0;
    }
                                ...
                                struct arm_instruction *instruction, char *cp)
{
    ...
    case 0:
        ...
        pld_literal:
            ...
            if (opcode & (1 << 23))
                ...
                address -= immed;
            sprintf(cp, "PLD\tr%d, %#8.8x", rt, address);
            return 0;
        }
        if (rn == 0x0f && rt != 0x0f) {
        ldrb_literal:
            ...
            if (opcode & (1 << 23))
                ...
                address -= immed;
            sprintf(cp, "LDRB\tr%d, %#8.8x", rt, address);
            return 0;
        }
        if (rn == 0x0f)
            break;
        if ((op2 & 0x3c) == 0x38) {
            immed = opcode & 0xff;
            sprintf(cp, "LDRBT\tr%d, [r%d, #%d]\t; %#2.2x", rt, rn, immed, immed);
            return 0;
        }
        if ((op2 & 0x3c) == 0x30) {
            if (rt == 0x0f) {
            ...
            preload_immediate:
                p1 = (opcode & (1 << 21)) ? "W" : "";
                sprintf(cp, "PLD%s\t[r%d, #%d]\t; %#6.6x", p1, rn, immed, immed);
                return 0;
            }
            mnemonic = "LDRB";
        ldrxb_immediate_t3:
        ...
        ldrxb_immediate_t2:
            sprintf(cp, "%s\tr%d, [r%d%s, #%d%s\t; %#8.8x", mnemonic, rt, rn, p1, immed, p2, immed);
            return 0;
        }
        if ((op2 & 0x24) == 0x24) {
            mnemonic = "LDRB";
    ...
    case 2:
        ...
        if ((op2 & 0x3c) == 0x38) {
            immed = opcode & 0xff;
            sprintf(cp, "LDRSBT\tr%d, [r%d, #%d]\t; %#2.2x", rt, rn, immed, immed);
            return 0;
        }
        if ((op2 & 0x3c) == 0x30) {
            if (rt == 0xf) {
                immed = opcode & 0xff;
                immed = -immed; // pli
                sprintf(cp, "PLI\t[r%d, #%d]\t; -%#2.2x", rn, immed, -immed);
                return 0;
            }
            mnemonic = "LDRSB";
            goto ldrxb_immediate_t3;
    ...
    case 3:
        if (rt == 0xf) {
            immed = opcode & 0xfff;
            sprintf(cp, "PLI\t[r%d, #%d]\t; %#3.3x", rn, immed, immed);
            return 0;
        }
        if (rn == 0xf)
            goto ldrsb_literal;
    ...
    if ((opcode & (1 << 23)) == 0) {
        ...
        ldrh_literal:
            ...
            if (opcode & (1 << 23))
                ...
                address -= immed;
            sprintf(cp, "LDR%sH\tr%d, %#8.8x", sign, rt, address);
            return 0;
        }
        if (op2 == 0) {
            int rm = opcode & 0xf;
            ...
            immed = (opcode >> 4) & 0x3;
            sprintf(cp, "LDR%sH.W\tr%d, [r%d, r%d, LSL #%d]", sign, rt, rn, rm, immed);
            return 0;
        }
        if ((op2 & 0x3c) == 0x38) {
            immed = opcode & 0xff;
            sprintf(cp, "LDR%sHT\tr%d, [r%d, #%d]\t; %#2.2x", sign, rt, rn, immed, immed);
            return 0;
        }
        if ((op2 & 0x3c) == 0x30 || (op2 & 0x24) == 0x24) {
            ...
            sprintf(cp, "LDR%sH\tr%d, [r%d%s, #%d%s\t; %#8.8x", sign, rt, rn, p1, immed, p2, immed);
            return 0;
        }
    } else {
        if (rn == 0xf)
        ...
        immed = opcode & 0xfff;
        sprintf(cp, "LDR%sH%s\tr%d, [r%d, #%d]\t; %#6.6x", sign, *sign ? "" : ".W", rt, rn, immed,
                immed);
        return 0;
    }
 ...
 */
int thumb2_opcode(unsigned address, struct arm_instruction *instruction,
                  unsigned short (*target_read_u16)(unsigned))
{
    int retval;
    unsigned short op;
    unsigned opcode;
    ...
    uerror("Can't decode 32-bit Thumb2 yet (opcode=%08x)", opcode);
...
int arm_access_size(struct arm_instruction *instruction)
{
    if ((instruction->type == ARM_LDRB) || (instruction->type == ARM_LDRBT) ||
        (instruction->type == ARM_LDRSB) || (instruction->type == ARM_STRB) ||
        (instruction->type == ARM_STRBT)) {
        return 1;
    } else if ((instruction->type == ARM_LDRH) || (instruction->type == ARM_LDRSH) ||
               (instruction->type == ARM_STRH)) {
        return 2;
    } else if ((instruction->type == ARM_LDR) || (instruction->type == ARM_LDRT) ||
               (instruction->type == ARM_STR) || (instruction->type == ARM_STRT)) {
        return 4;
    } else if ((instruction->type == ARM_LDRD) || (instruction->type == ARM_STRD)) {
        return 8;
    } else {
        uerror("BUG: instruction type %i isn't a load/store instruction", instruction->type);
        return 0;
    }
}
...
std::string arm_disassemble(uint32_t opcode)
{
    ...
    while (std::isspace(*result.rbegin()))
