...
//
Vtty::Vtty(const std::string &n, int p) : name(n), tcp_port(p)
{
...
{
    if (tcp_port != 0) {
        if (tcp_state == Tcp_State::RUNNING && fwrite(&ch, 1, 1, output_stream) != 1) {
...
void Vtty::read_and_store()
{
    ...
    case Input_State::TELNET_SB2:
        telnet_qual = c;
        if ((telnet_opt == TELOPT_TTYPE) && (telnet_qual == TELQUAL_IS))
...
int Vtty::tcp_conn_accept()
{
    ...
    if (fd < 0) {
        fprintf(stderr, "tcp_conn_accept: accept on port %d failed %s\n", tcp_port,
                strerror(errno));
        return -1;
    }

    std::cerr << name << " is now connected (accept_fd=" << accept_fd << ", conn_fd=" << fd << ")"
              << std::endl;
...
int Vtty::tcp_conn_wait()
{
    ...
    std::cerr << name << ": waiting connection on tcp port " << tcp_port << " (FD " << accept_fd
              << ")" << std::endl;
...
int Vtty::wait(fd_set *rfdp)
{
    ...
    if (res == -1) {
        if (errno != EINTR) {
            ...
            for (auto vtty = Vtty::first; vtty != nullptr; vtty = vtty->next) {
                std::cerr << "   " << vtty->name << ": FD " << vtty->fd << std::endl;
            }
        }
        return 0;
    }
